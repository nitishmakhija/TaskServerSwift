...
class TestTasks {
    ...
    public class func seed(databaseContext: DatabaseContext) throws {
        ...
        for index in 1...10 {
            tasks.append(Task(id: UUID(), createDate: Date(), name: "John \(index) task", isFinished: false, userId: TestUsers.getJohnDoe().id))
        }

        for index in 1...10 {
            tasks.append(Task(id: UUID(), createDate: Date(), name: "Viki \(index) task", isFinished: false, userId: TestUsers.getVikiDoe().id))
        }
